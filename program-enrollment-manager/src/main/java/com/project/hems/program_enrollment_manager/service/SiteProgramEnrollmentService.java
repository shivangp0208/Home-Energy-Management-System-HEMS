...
public class SiteProgramEnrollmentService {
    ...
    public SiteEnrollSuccessResponse enrollSiteinProgram(UUID siteId, UUID programId) {
        ...
        ProgramEntity programEntity = programRepository.findById(programId)
                .orElseThrow(() -> new RuntimeException(" program is not found"));

        // now we check program start and end time ke valid che date ni range ma che ke
        // nai
        LocalDate now = LocalDate.now();
        // LocalDate endDateTime = programEntity.getEndDateTime();;
        if (now.isBefore(programEntity.getStartDate()) || now.isAfter(programEntity.getEndDate())) {
