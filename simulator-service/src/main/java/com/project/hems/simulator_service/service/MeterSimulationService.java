...
public class MeterSimulationService {
    ...
    public void simulateLiveReadings() {
        ...
        if (meterReadings.isEmpty()) {
            log.warn("simulateLiveReadings: meterReadings map is empty, loading initial values from DB");
            meterManagementService.getValuesFromDB();
            return;
        }

        log.info("simulateLiveReadings: starting simulation cycle for {} meters", meterReadings.size());
        ...
        for (Map.Entry<UUID, MeterSnapshot> entry : meterReadings.entrySet()) {
            ...
            log.debug("simulateLiveReadings: simulating meter for siteId={}, meterId={}", siteId, meter.getMeterId());
            ...
            log.debug("simulateLiveReadings: siteId={} solarW={}W loadW={}W", siteId, solarW, loadW);

            // 2. Physics Engine (Priority Logic)
            Optional<DeviceState> deviceState = deviceCommandStore.getEventState(siteId);

            List<EnergyPriority> loadpriorities = deviceState.map(DeviceState::getLoadEnergyPriorities).orElse(DeviceCommandStore.loadEnergyPriorities);
            List<EnergyPriority> surpluspriorities = deviceState.map(DeviceState::getLoadEnergyPriorities).orElse(DeviceCommandStore.loadEnergyPriorities);
            meter.setLoadEnergyPriorities(loadpriorities);
            meter.setSurplusEnergyPriorities(surpluspriorities);

            energyPhysicsEngine.processEnergyBalance(meter, solarW, loadW, loadpriorities, surpluspriorities, deviceState.orElse(null));


            log.debug("simulateLiveReadings: siteId={} after physics batteryPowerW={} gridPowerW={}", siteId, meter.getBatteryPowerW(), meter.getGridPowerW());
            ...
            if (invalidCapacity) {
                log.warn("simulateLiveReadings: siteId={} invalid batteryCapacityWh={}, forcing SOC=0", siteId, meter.getBatteryCapacityWh());
            }

            meter.setBatterySoc(invalidCapacity ? 0 : (int) Math.round((meter.getBatteryRemainingWh() / meter.getBatteryCapacityWh()) * 100));

            meter.setTimestamp(LocalDateTime.now());

            log.debug("simulateLiveReadings: siteId={} SOC={}%, voltage={}V current={}A", siteId, meter.getBatterySoc(), meter.getCurrentVoltage(), meter.getCurrentAmps());
